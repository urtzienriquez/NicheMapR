...
C     MKS SYSTEM OF UNITS (J, M, KG, S, C, K, PA)
     ...
     & INTENT(IN) :: S_INSTAR1
      ...
      DOUBLE PRECISION MSHSOI,MSOIL,MU_E,MU_P,MU_V,MU_X,N2GAS,NEWCLUTCH
      DOUBLE PRECISION NEWDEP,NTRY1,NWASTE,O2FLUX,O2GAS
      ...
      DIMENSION DEBMOD1(96),DEBQMET(24),DEP1(10),DEPSEL(25),DRYFOOD(24)
      DIMENSION E_BABY1(24),E_H(24),E_HPUP(24),ECTOINPUT(133)
      DIMENSION ECTOINPUT1(133),ED(24),EGGSHSOI(25),EGGSOIL(25)
      ...
      COMMON/BUR/NON,MINNODE,BURROWTMP,BURROWWTR
     ...
     & W_V,E_EGG,KAP_V,KAP_X,KAP_X_P,MU_X,MU_P,W_N,W_P,W_X,FUNCT
...
C    CHECK IF BUCKET MODEL HAS RUN (OR IF WET0D WAS RUN) AND, IF SO, SWITCH OFF POND
     ...
     &  (ECTOINPUT1(116).EQ.1))THEN
       ...
       BURROWTMP=INT(ECTOINPUT1(132))
       BURROWWTR=INT(ECTOINPUT1(133))
...
233     CONTINUE
     ...
     &     (SUM(ACTHR).EQ.0)))THEN ! NO ACTIVITY YET FOR THE DAY
         ...
         IF(BURROWSHADE.EQ.1)THEN
          SHADE=MAXSHD
          CALL SELDEP(TSHSOI,HSHSOI,ZSOIL,RELHUM,PSHSOI)
         ELSE
          SHADE=REFSHD
          CALL SELDEP(TSOIL,HSOIL,ZSOIL,RELHUM,PSOIL)
         ENDIF
...
C      CHECK FOR NO SUNLIGHT
       IF(QSOLRF.EQ.0.0000) THEN
C       NIGHTTIME: ACTIVITY OK
        IF((RH(IHOUR).GE.80).OR.(PSOIL(MINNODE).GE.-72.5))THEN
         IF((RAINACT.EQ.'Y').OR.(RAINACT.EQ.'Y'))THEN
          IF(RAINFALL.GT.RAINTHRESH)THEN
           CALL ABOVEGROUND
           GOTO 253
...
C           NO BURROW ALLOWED
         ...
         GOTO 253
        ELSE
C      TOO DRY CONDITIONS, PUT IN BURROW
         IF((BURROW.EQ.'Y').OR.(BURROW.EQ.'Y'))THEN
C         PUT ANIMAL IN BURROW, IT'S NIGHTTIME
          IF(BURROWSHADE.EQ.1)THEN
           SHADE=MAXSHD
